...
class EditNova extends SpotlightCommand
{
    protected $query;
    protected $links;
     ...
     */
    protected string $description = 'Edit Nova Resources';
    ...
    public function searchResources($query)
    {
        ...
        foreach (session()->get('resources') as $resourceClass) {
            ...
            $resources[] = new SpotlightSearchResult(
                $resourceClass,
                $resourceTitle,
                sprintf('Edit %s Resource ', $resourceTitle)
            );
        }
     ...
     */
    public function execute(Spotlight $spotlight, $resource, $type = 'edit')
    {
        $links = session()->get('links');
        $link = $links[$resource] . '/edit';
    ...
    protected function transformResult($resourceClass, Resource $resource)
    {
        ...
        return new SpotlightSearchResult(
            $model->getKey(),
            $resource->title(),
            sprintf('Edit %s', $resourceClass::label())
        );
    }
